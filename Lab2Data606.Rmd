...
All three histograms all convey the same data. The difference between the three, are the width of the bins. The larger bins probably are easier to see, but the smaller bins show with more detail that most flights that were delayed, were only delayed a little bit. (Maybe 10-15 minutes)    
...
sfo_feb_flights %>%
  summarise(n         = n())
```
There were 68 flights that headed to SFO in February. 
...
```
The histogram is right-skewed and shows that a lot of the flights in February headed to SFO actually had early arrivals. (negative numbers = early arrival)  
...
```
Both Delta and United Airlines are tied for the most variable arrival delays. 
...
```
July looks to be the worst month. Expect the most delays then. (June doesn't look great either)
...
```
Lowest Mean Dept - September
Lowest Median Dept - November
The mean would be more reliable most of the time as it gives us the actual average of all delays. The mean includes all the values in the data set for its calculation, and any change in any of the scores will affect the value of the mean. This is not the case with the median. For data with skewed distribution though, the median is better than the mean because it isn't influenced by extremely large values.
...
Laguardia has the best time departure percentage of 72.8%. 
...
ggplot(nycflights, aes(x = distance, y = avg_speed)) +
  geom_point()
```
There seems to be a mostly positive association between distance and average speed. 
...
ggplot(data = three_carriers, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()
```
