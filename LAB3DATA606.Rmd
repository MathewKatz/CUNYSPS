...
**A streak length of one means that the shooter hit one basket before missing the next shot. A streak length of zero means that the shooter missed a shot or missed consecutive shots.**
...
**The distribution of Kobe's shot streaks are right-skewed where almost all of the data has his streak lenghts at 1 or 0. His typical streak length is "0" which happened around 38 times. His longest streak of baskets was "4" and it happened once.**
...
table(unfair_coin)
```
**In my simulation of flipping the unfair coin 100 times, I flipped a heads, fourteen times and a tails, eighty six times.**
...
ggplot(data=sim_streak,aes(x=length))+
  geom_bar()
````
**The distribution is skewed right again. The most occuring streak was "0" again. The player's longest streak of baskets in 133 shots was 6. Not too shabby.**
...
**I feel that it is very likely the streak distribution would be similar but not necessarily exactly the same. The only thing that is guaranteed is that the shooter will hit around 59.85 shots (45% of 133 shots.) There are many different orders to hitting 59.85 out of 133 shots but for the most part the distribution should look right skewed.**
...
**They are both right skewed and very similar. There doesn’t appear to be evidence for Kobe Bryant’s hot hand. All shots are independent of each other. When the shooter hits a shot, his likelihood of hitting another shot in a row shouldn't change that much from his initial shooting percentage. **
